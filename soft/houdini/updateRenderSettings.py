...
def main():
    ...
    #get file version:
    existing_parm = updateNode.parm("version")
    if existing_parm is None:
        fileversion=0
        version_label_parm_template = hou.IntParmTemplate("version","version_label",1,default_value=([0]))
        parm_group.append(version_label_parm_template)
        updateNode.setParmTemplateGroup(parm_group)
    else:
        fileversion=existing_parm.eval()
    ...
    if fileversion<=1:
        if fileversion==0:
            ...
            updateNode.parm("version").set(1)

        #version 2 modifications
        hou.node("/stage/aov/AOVs").parm("switch_Z").set(0)
        print("desactiving Z because its already on the fog renderlayer")
        updateNode.parm("version").set(2)
...
def create_node(name,type,path,input,output):
    print ("creating node ") 
    node=hou.node(path).createNode(type, name)
    inputPos=input.position()
    node.move((inputPos[0],inputPos[1]-0.5))
